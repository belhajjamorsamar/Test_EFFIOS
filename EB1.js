...
fetch("https://data.education.gouv.fr/api/records/1.0/search/?dataset=fr-en-dnma-par-uai-appareils&q=&facet=debutsemaine&facet=uai&refine.uai=0010024W").then((data)=>{
        return data.json();
    }).then((objectData)=>{
     ...
     const word="visites";    
      for (let i = 0; i <=10; i++) { 
               ...
               //console.log(Object1);
               var s=0;
        for(const key of Object.keys(Object1)){//parcour  chaque  key de chaque objet pour tester si il conteinet un mot visites
              let position =key.indexOf(word);
              if(position===0){//càd le key contient le mot visites
              var v = parseInt(Object1[key]);//je vais convertir chaque value string en entier 
